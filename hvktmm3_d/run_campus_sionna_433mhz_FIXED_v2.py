...
"""
SIONNA RT - MÔ PHỎNG KÊNH 433 MHz TRÊN MÔ HÌNH KHUÔN VIÊN TRƯỜNG

File mô hình mặc định:
    campus_chinh_chieu_cao.glb

File vật liệu đi kèm:
    apply_campus_radio_materials_433mhz_FIXED.py
    campus_radio_materials_433mhz.xml

Script này:
- Tự đổi tên object kiểu Buildings.006 -> Buildings_006 để tương thích Mitsuba.
1) Đọc trực tiếp file GLB bằng trimesh.
2) Tách từng object trong GLB thành mesh tam giác sạch để Sionna RT đọc.
3) Giữ tên object như Buildings*, Ground_Aerial, Roads_Paved*,
   Tree_Trunks*, Tree_Canopies*, Boundary_Walls*, ATM_Kiosk.
4) Tự sửa hệ trục GLB để Z là chiều cao.
5) Gán custom RadioMaterial RF tương ứng ở 433 MHz.
5) Đặt TX/RX mặc định cách nhau 100 m, cao 2 m trên Ground (Z=0).
6) Bật LOS, phản xạ gương, phản xạ khuếch tán,
   truyền xuyên/refraction, nhiễu xạ và edge diffraction.
7) In thông số kênh ra Terminal và ghi CSV.

Backend:
    CPU LLVM trên Windows để tránh lỗi OptiX/PTX đã gặp trước đó.

Cài thêm nếu thiếu:
    python -m pip install sionna-rt numpy trimesh

Chạy thử nhẹ:
    python run_campus_sionna_433mhz.py --samples 20000 --max-depth 3

Chạy 100000 samples:
    python run_campus_sionna_433mhz.py --samples 100000 --max-depth 5
"""
...
# ============================================================
# WINDOWS: ÉP DR.JIT / MITSUBA CHẠY CPU LLVM
# ============================================================
...
try:
    ...
    print("[LỖI] Chưa cài trimesh.")
    print("      Chạy: python -m pip install trimesh")
    raise
...
try:
    ...
    print("      Chạy: python -m pip install sionna-rt numpy")
    raise

try:
    from apply_campus_radio_materials_433mhz_FIXED import MATERIALS, classify
except ImportError:
    print("[LỖI] Không tìm thấy apply_campus_radio_materials_433mhz_FIXED.py")
    print("      Hãy đặt file vật liệu FIXED cùng thư mục với script này.")
    raise
...
# ============================================================
# TẠO SCENE SIONNA TỪ GLB
# ============================================================

def _safe_filename(name: str) -> str:
    # Mitsuba dùng "." làm delimiter trong object path, nên tên object
    # không được chứa dấu chấm. Ví dụ Buildings.006 -> Buildings_006
    name = name.replace(".", "_")
    name = re.sub(r"[^A-Za-z0-9_-]+", "_", name)
    return name.strip("_-") or "mesh"


def build_scene(glb_path: Path, cache_dir: Path):
    print_title("1. NẠP MÔ HÌNH 3D + SỬA HỆ TRỤC + GÁN VẬT LIỆU")

    if not glb_path.exists():
        raise FileNotFoundError(
            f"Không thấy file GLB:\n  {glb_path}\n"
            "Hãy đặt campus_chinh_chieu_cao.glb cùng thư mục với script."
        )

    print(f"[GLB] Đang đọc: {glb_path}")
    source = trimesh.load(str(glb_path), force="scene")

    if not isinstance(source, trimesh.Scene):
        tmp = trimesh.Scene()
        tmp.add_geometry(source)
        source = tmp

    print(f"[GLB] Số geometry : {len(source.geometry)}")
    print(f"[GLB] BBox GỐC min: {np.asarray(source.bounds[0])}")
    print(f"[GLB] BBox GỐC max: {np.asarray(source.bounds[1])}")
    print(f"[GLB] Size GỐC    : {np.asarray(source.bounds[1]) - np.asarray(source.bounds[0])} m")

    # File GLB hiện tại có:
    #   X ≈ 190 m
    #   Y ≈  51 m  <- chiều cao
    #   Z ≈ 105 m  <- chiều rộng khuôn viên
    #
    # Sionna cần:
    #   X/Y = mặt đất
    #   Z   = chiều cao
    ...
    # Custom RadioMaterial tại 433 MHz.
    # Không dùng ITURadioMaterial vì concrete/ground/metal ngoài dải ITU.
    radio_materials = {}
    for mat_name, kwargs in MATERIALS.items():
        mat = RadioMaterial(name=mat_name, **kwargs)
        scene.add(mat)
        radio_materials[mat_name] = mat

    # dump() bake world transform từ GLB trước khi ta sửa trục.
    meshes = source.dump(concatenate=False)
    ...
    for idx, mesh in enumerate(meshes):
        ...
        meta_name = str(
            mesh.metadata.get("node")
            or mesh.metadata.get("name")
            or f"mesh_{idx:03d}"
        )

        used_names[meta_name] += 1
        if used_names[meta_name] == 1:
            obj_name = meta_name
        else:
            obj_name = f"{meta_name}_{used_names[meta_name]:02d}"

        mat_name = classify(obj_name)
        if mat_name is None:
            geom_name = str(mesh.metadata.get("name") or "")
            mat_name = classify(geom_name)

        if mat_name is None:
            skipped.append(obj_name)
            print(f"[CẢNH BÁO] Bỏ qua object chưa có mapping vật liệu: {obj_name}")
            continue
        ...
        # SỬA HỆ TRỤC GLB -> SIONNA
        clean.apply_transform(rot_x_90)
        clean.remove_unreferenced_vertices()

        bounds = clean.bounds
        all_min.append(bounds[0])
        all_max.append(bounds[1])

        safe_obj_name = _safe_filename(obj_name)

        mesh_file = cache_dir / f"{idx:03d}_{safe_obj_name}.ply"
        clean.export(str(mesh_file))

        scene_obj = SceneObject(
            fname=str(mesh_file),
            name=safe_obj_name,
            radio_material=radio_materials[mat_name],
            remove_duplicate_vertices=True,
        )
        scene.edit(add=scene_obj)

        material_counts[mat_name] += 1
        added += 1

        if safe_obj_name != obj_name:
            print(f"[ĐỔI TÊN] {obj_name} -> {safe_obj_name}")

        print(
            f"[OBJECT] {safe_obj_name:28s} -> {mat_name:22s} "
            f"| V={len(clean.vertices):5d} F={len(clean.faces):5d}"
        )

    if added == 0:
        raise RuntimeError("Không có mesh nào được thêm vào scene Sionna.")

    bmin = np.min(np.vstack(all_min), axis=0)
    bmax = np.max(np.vstack(all_max), axis=0)
    ...
    print(f"[SCENE] Tâm         : {center}")
    print("[SCENE] Quy ước      : X/Y = mặt đất | Z = chiều cao")

    print("\n--- TỔNG HỢP VẬT LIỆU 433 MHz ---")
    for mat_name, count in sorted(material_counts.items()):
        m = MATERIALS[mat_name]
        print(
            f"{mat_name:24s}: {count:3d} object | "
            f"eps_r={m['relative_permittivity']:.4g} | "
            f"sigma={m['conductivity']:.6g} S/m | "
            f"S={m['scattering_coefficient']:.2f}"
        )

    if skipped:
        print("[CẢNH BÁO] Object chưa được gán:", ", ".join(skipped))

    print(f"[RF] f_c          = {scalar_float(scene.frequency)/1e6:.3f} MHz")
    print(f"[RF] lambda       = {scalar_float(scene.wavelength):.6f} m")
    print(f"[RF] Tổng object  = {added}")
    print(f"[CACHE] Mesh sạch = {cache_dir}")

    return scene, bmin, bmax, center
...
def configure_radio_devices(
    ...
    if tx_override is not None and rx_override is not None:
        ...
        # GLB hiện tại đã chuẩn hóa: X/Y là mặt đất, Z là chiều cao.
        # Đặt TX/RX quanh tâm khuôn viên, cách nhau theo trục X.
        z = max(0.0, float(bmin[2])) + height_m
...
def analyze_paths(
    ...
    if len(valid_idx) == 0:
        ...
        print("Có thể TX/RX nằm trong vật thể, samples quá thấp, hoặc vị trí hiện tại chưa phù hợp.")
        return
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Sionna RT 433 MHz - Campus full channel test"
    )
    p.add_argument(
        "--glb",
        default="campus_chinh_chieu_cao.glb",
        help="File GLB khuôn viên trường",
    )
    p.add_argument(
        "--cache-dir",
        default="sionna_meshes_cache",
        help="Thư mục mesh tạm được tách từ GLB",
    )
    p.add_argument(
        "--distance",
        type=float,
        default=100.0,
        help="Khoảng cách TX-RX tự động [m]",
    )
    p.add_argument(
        "--height",
        type=float,
        default=2.0,
        help="Độ cao TX/RX so với Ground Z=0 [m]",
    )
    p.add_argument(
        "--samples",
        type=int,
        default=20_000,
        help="samples_per_src (CPU: nên test 20k trước)",
    )
    p.add_argument(
        "--max-depth",
        type=int,
        default=3,
        help="Số tương tác tối đa",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=30.0,
        help="Công suất TX [dBm], mặc định rBS = 30 dBm",
    )
    p.add_argument(
        "--bandwidth-hz",
        type=float,
        default=500_000.0,
        help="Băng thông kênh [Hz]",
    )
    p.add_argument(
        "--noise-figure-db",
        type=float,
        default=6.0,
        help="Noise figure giả định [dB]",
    )
    p.add_argument(
        "--tx",
        type=float,
    ...
    p.add_argument(
        "--csv",
        default="campus_channel_paths_433mhz.csv",
        help="CSV chi tiết multipath",
    )
    return p.parse_args()
...
def main():
    ...
    print_title("SIONNA RT - KHUÔN VIÊN TRƯỜNG / 433 MHz")
    print(f"Python          : {sys.version.split()[0]}")
    print(f"Sionna RT       : {rt.__file__}")
    print(f"Mitsuba variant : {mi.variant()}")
    if sys.platform.startswith("win"):
        print(f"LLVM-C.dll      : {os.environ.get('DRJIT_LIBLLVM_PATH')}")
    print(f"GLB             : {glb_path.name}")
    print("Material helper : apply_campus_radio_materials_433mhz_FIXED.py")
